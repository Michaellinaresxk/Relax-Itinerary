...
const emptyFlight = (): Flight => ({
  airline: '', code: '', date: '', time: '', origin: '', destination: '',
})

const initial = (): FormData => ({
  mainGuest: '', email: '', phone: '',
  adults: 2, children: 0, childAges: [],
  guests: [],
  checkIn: '', checkOut: '',
  arrivalFlights: [emptyFlight()],
  departureFlights: [emptyFlight()],
  needsTransfer: null, bags: 0, transferNotes: '',
  dayActivities: {},
  equipment: [],
  dayMeals: {},
  wantGrocery: null, groceryNotes: '',
...
export function useFormData() {
  ...
  function updateChildAge(i: number, v: string) { state.childAges[i] = v }

  /* ── Guests ────────────────────────────── */
  function addGuest() { state.guests.push({ name: '', relation: '' }) }
  function removeGuest(i: number) { state.guests.splice(i, 1) }
  function updateGuest(i: number, f: keyof Guest, v: string) { state.guests[i][f] = v }
  ...
  function updateActivity(dayKey: string, actId: string, field: keyof SelectedActivity, value: string | number) {
  ...
  function resetForm() { Object.assign(state, initial()) }

  return {
    state, updateField,
    setChildren, updateChildAge,
    addGuest, removeGuest, updateGuest,
    addFlight, removeFlight, updateFlight,
    toggleActivity, updateActivity,
    toggleEquipment, updateEquipmentQty,
    updateMeal, resetForm,
  }
}
